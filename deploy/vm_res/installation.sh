...
sudo su -c "docker login registry.gitlab.com -u votingly-deploy-token --password-stdin <<< $(docker run -ti google/cloud-sdk:alpine gcloud secrets versions access latest --secret="GITLAB_DEPLOY_TOKEN" --project votingly)" admin
